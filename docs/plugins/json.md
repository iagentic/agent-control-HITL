...
>
> **JSON Schema can handle ALL the validation checks** this plugin provides:
...
2. **Plugin validates** → Returns error if validation fails
...
- **regex plugin** - Simple pattern matching without JSON structure validation
- **list plugin** - Check if values are in/not in a list
