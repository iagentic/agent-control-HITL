...
test.describe("JSON Evaluator", () => {
  test("displays schema validation section", async ({ mockedPage }) => {
    ...
    await expect(mockedPage.getByText("JSON Schema", { exact: true })).toBeVisible();
    await expect(
  ...
  test("displays field validation section", async ({ mockedPage }) => {
    ...
    await expect(mockedPage.getByText("Required fields", { exact: true })).toBeVisible();
    await expect(mockedPage.getByText("Field types", { exact: true })).toBeVisible();
    await expect(mockedPage.getByText("Field constraints", { exact: true })).toBeVisible();
    await expect(mockedPage.getByText("Field patterns", { exact: true })).toBeVisible();
  });
  ...
  test("displays validation behavior checkboxes", async ({ mockedPage }) => {
    ...
    await expect(mockedPage.getByText("Allow null for required fields")).toBeVisible();
  ...
  test("can enter JSON schema", async ({ mockedPage }) => {
    ...
    await expect(schemaTextarea).toHaveValue('{"type": "object", "required": ["name"]}');
  });

  test("can toggle allow extra fields", async ({ mockedPage }) => {
    await openEvaluatorForm(mockedPage, "JSON");

    const checkbox = mockedPage.getByRole("checkbox", { name: "Allow extra fields" });
