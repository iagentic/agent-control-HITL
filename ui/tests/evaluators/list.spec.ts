...
test.describe("List Evaluator", () => {
  test("displays all list form fields", async ({ mockedPage }) => {
    ...
    await expect(mockedPage.getByPlaceholder("Enter values (one per line)")).toBeVisible();

    // Logic select
    await expect(mockedPage.getByText("Logic", { exact: true })).toBeVisible();

    // Match on select
    await expect(mockedPage.getByText("Match on", { exact: true })).toBeVisible();

    // Match mode select
    await expect(mockedPage.getByText("Match mode", { exact: true })).toBeVisible();
  ...
  test("can enter multiple values", async ({ mockedPage }) => {
    ...
    const valuesTextarea = mockedPage.getByPlaceholder("Enter values (one per line)");
  ...
  test("can toggle case sensitive", async ({ mockedPage }) => {
    ...
    const checkbox = mockedPage.getByRole("checkbox", { name: "Case sensitive" });
