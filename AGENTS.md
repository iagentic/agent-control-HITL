...
- `models/`: shared Pydantic v2 models (`models/src/agent_control_models/`)
- `engine/`: control evaluation engine (`engine/src/agent_control_engine/`)
- `server/`: FastAPI server (`server/src/agent_control_server/`)
- `sdks/python/`: Python SDK (`sdks/python/src/agent_control/`)
...
- Add a new evaluator:
  1) implement in `engine/src/agent_control_engine/`
  2) register in the engine registry
  3) add tests in the engine package
  4) ensure server/SDK integration (if applicable)
