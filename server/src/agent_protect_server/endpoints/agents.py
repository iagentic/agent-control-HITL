...
@router.post(
    ...
    response_description="Agent registration status with active rules",
)
async def init_agent(
    ...
    Returns:
        InitAgentResponse with created flag and active rules (if policy assigned)
    ...
    # If the existing agent has a policy, include its rules; otherwise empty list
    rules = []
    if existing.policy_id is not None:
        rules = await list_rules_for_agent(existing.agent_uuid, db)

    return InitAgentResponse(created=created, rules=rules)
...
async def set_agent_policy(
    ...
    The agent will immediately inherit all rules from controls in the assigned policy.
...
async def delete_agent_policy(
    ...
    The agent will no longer have any protection rules active.
...
@router.get(
    ...
    summary="List agent's active rules",
    response_description="List of rules from agent's policy",
)
async def list_agent_rules(
    agent_id: UUID, db: AsyncSession = Depends(get_async_db)
) -> AgentRulesResponse:
    """
    List all protection rules active for an agent.

    Rules are inherited from all controls in the agent's assigned policy.
    ...
    Returns:
        AgentRulesResponse with list of rules (empty if no policy)
    ...
    if agent.policy_id is None:
        return AgentRulesResponse(rules=[])

    rules = await list_rules_for_agent(agent_id, db)
    return AgentRulesResponse(rules=rules)
